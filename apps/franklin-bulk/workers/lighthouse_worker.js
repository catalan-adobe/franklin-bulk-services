...
parentPort.on('message', async (msg) => {
  if (msg && msg.type === 'exit') {
    ...
    try {
      ...
      const result = await importerLib.Puppeteer.runStepsSequence(page, msg.url, 
        [
      ...
      parentPort.postMessage({
        ...
        passed: true,
        result: 'Success',
        preMsg: `LH (${result.lighthouse.version}) Scores: ${Object.keys(result.lighthouse.scores).map(k => { return `${k}: ${result.lighthouse.scores[k].score}` }).join(', ')} `,
      });
    } catch (error) {
      parentPort.postMessage({
        url: msg.url,
